...
static void
print_operands (const struct arc_opcode *opcode,
                bfd_vma memaddr,
                uint64_t insn,
                uint32_t isa_mask,
                insn_t *pinsn,
              ...
              struct disassemble_info *info)
{
  ...
  insn_len = arc_insn_length ((buffer[highbyte] << 8 |  buffer[lowbyte]), *isa_mask);
  ...
  switch (insn_len)
	...
	*insn =
	  ((((uint64_t) ARRANGE_ENDIAN (info, buffer)) << 32)
...
int
print_insn_arc (bfd_vma memaddr, struct disassemble_info *info)
{
